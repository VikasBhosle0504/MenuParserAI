 ...
 */
async function processMenuWithLangchainVision(ocrText, docId, cloudFunctionUrl = 'https://YOUR_CLOUD_FUNCTION_URL/parse-menu', sourceFilePath) {
  try {
    ...
    const response = await axios.post(cloudFunctionUrl, { ocr_data: menuText , docId: docId, sourceFilePath: sourceFilePath});
 ...
 */
const processUploadedFileWithLangchainVision = functions
.region('us-central1')
...
.onFinalize(async (object) => {
  ...
  if (!filePath.startsWith('menus_langchain_vision/') || !SUPPORTED_EXTENSIONS.includes(extension)) {
    return;
  }
  ...
  try {
    ...
    rawTextPath = `debug_langchain_vision/${rawTextFileName}`;
    ...
    processMenuWithLangchainVision(
      rawText,
      fileName,
      'https://menu-parser-api-v2-uigq6w2r7a-uc.a.run.app/parse-menu-from-file',
      filePath
    ).catch(err => {
