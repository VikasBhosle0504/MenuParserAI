...
function uploadHandler(e) {
  ...
  const storageRef = storage.ref(`menus_langchain/${uniqueName}`);
...
function pollForProcessedMenu(docId, attempt) {
  ...
  db.collection('menus_langchain').doc(docId).get().then((doc) => {
    if (doc.exists) {
...
async function loadMenus(direction = null) {
  ...
  let query = db.collection('menus_langchain').orderBy('createdAt', 'desc').limit(pageSize);
